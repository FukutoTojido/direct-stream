	...
	useEffect(() => {
		const getData = async () => {
			...
			try {
				...
				const { username, avatar, id, global_name } =
					await oauth.getUser(accessToken);

				let [nick, guildAvatar]: [
					nick?: string | null,
				...
				try {
					const { nick: nick_, avatar: guildAvatar_ } =
						await oauth.getGuildMember(accessToken, "228205151981273088");
					nick = nick_;
				...
				setState({
					...
					id,
					isJoinedServer,
					isGuildAvatar: guildAvatar !== null && guildAvatar !== undefined,
				});
			} catch (error) {
				console.log(error);
				setState(null);
